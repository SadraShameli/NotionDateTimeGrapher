...
def plot_graph():
    start_time_counts_sorted = dict(
        sorted(find_missing_numbers(start_time_counts, 24).items())
    )
    end_time_counts_sorted = dict(
        sorted(find_missing_numbers(end_time_counts, 24).items())
    )
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))

    ax1.bar(
        start_time_counts_sorted.keys(), start_time_counts_sorted.values(), color="blue"
    )
